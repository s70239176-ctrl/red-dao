...
app.get('/api/health', (_req, res) => { res.json({ ok:true, version:'1.0.0', network:process.env.OPNET_NETWORK||'regtest', factory:process.env.FACTORY_ADDRESS||null, ts:Date.now() }) })
app.use(express.static(DIST_PUBLIC))
app.get('*', (_req, res) => { res.sendFile(path.join(DIST_PUBLIC, 'index.html')) })
app.listen(PORT, '0.0.0.0', () => { console.log(`[OPNet DAO] Running on :${PORT}`); console.log(`[OPNet DAO] Network : ${process.env.OPNET_NETWORK||'regtest'}`); console.log(`[OPNet DAO] Factory : ${process.env.FACTORY_ADDRESS||'(not set)'}`) })
